...
# Training data is optional for running the demo because trained model files are
# already included. If you collect/retrain data, keep it inside the project so it
# works on any laptop instead of one developer's old absolute Windows path.
...
def collect_data():
    # Create one folder per gesture and one subfolder per captured sequence.
    # Each sequence stores 10 .npy files, one for each frame's hand keypoints.
    ...
    # Capture keypoints from the webcam using MediaPipe Hands.
...
def append_data():
    # Load captured .npy sequences and convert labels to one-hot vectors for
    # Keras training.
    no_sequences = 190
